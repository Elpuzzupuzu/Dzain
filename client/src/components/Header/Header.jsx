...
const Header = ({ onCartToggle }) => {
  ...
  return (
    <>
      <header className="sticky top-0 bg-white shadow-sm z-50 font-sans border-b border-zinc-800">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4 gap-8">
            ...
            <Link to="/" className="flex items-center space-x-2 flex-shrink-0 group">
              <div className="bg-amber-700 text-white px-3 py-1 font-bold text-2xl transition-transform group-hover:scale-105">
                D
              </div>
              <span className="font-bold text-2xl tracking-tighter text-black">ZAIN</span>
            </Link>
            ...
            <div className="flex items-center gap-6">
              ...
              <button
                ...
                className={`flex items-center gap-2 group transition-colors ${
                  isCartAnimating ? 'text-amber-700' : 'text-black hover:text-amber-700'
                }`}
              >
                <div className="relative">
                  <ShoppingCart size={24} className={isCartAnimating ? 'animate-bounce' : ''} />
                  {totalCartItems > 0 && (
                    <span className="absolute -top-2 -right-2 bg-amber-700 text-white text-[10px] rounded-full w-5 h-5 flex items-center justify-center font-bold border-2 border-zinc-900">
                      {totalCartItems}
                    </span>
                  )}
                </div>
                <div className="hidden sm:flex flex-col items-start leading-none">
                  <span className="text-[10px] text-stone-400 uppercase font-bold tracking-wider">Mi Carrito</span>
                </div>
              </button>
              ...
              <button
                ...
                className="lg:hidden p-2 text-black hover:text-amber-700 transition-colors"
              >
                <circle />
                <Menu size={28} />
              </button>
          ...
          <div className="absolute left-0 top-0 bottom-0 bg-white w-72 max-w-[85vw] shadow-2xl flex flex-col">
            <div className="p-6 border-b border-stone-100 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <div className="bg-amber-700 text-white px-2 py-0.5 font-bold text-lg">D</div>
                <span className="font-bold text-lg text-zinc-900">ZAIN</span>
              </div>
              <button onClick={() => setIsMenuOpen(false)} className="text-stone-400 hover:text-zinc-900 transition-colors">
                <X size={24} />
              </button>
            </div>
