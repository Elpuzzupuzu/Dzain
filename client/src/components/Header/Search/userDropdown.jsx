...
const UserDropdown = ({
      ...
      {isLoggedIn && (
        <>
          {profilePicture ? (
            ...
            <div className="w-8 h-8 flex items-center justify-center bg-white text-zinc-900 font-bold text-sm rounded-full shadow-inner">
              {userInitials}
            </div>
          )}
        </>
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className={`absolute top-[calc(100%-5px)] right-0 bg-white shadow-2xl border border-stone-200 p-4 z-40 transform origin-top-right ${
            isLoggedIn ? "w-[450px] rounded-xl" : "w-[400px] rounded-lg"
          }`}
        >
          {/* Flecha decorativa del dropdown */}
          <div
            className={`absolute -top-3 right-6 w-4 h-4 bg-white transform rotate-45 border-t border-l ${
              isLoggedIn ? "border-stone-200 right-8" : "border-stone-200 right-4"
            }`}
          ></div>
          ...
          {!isLoggedIn ? (
            <div className="flex flex-col items-center p-2">
              ...
              <Link
                to="/login"
                onClick={handleLinkClick}
                className="w-full text-center bg-amber-700 hover:bg-amber-800 text-white font-bold py-2 rounded-md transition-colors shadow-md flex items-center justify-center gap-2"
              >
                <LogIn size={20} /> Iniciar Sesión
              </Link>
              ...
              <Link
                to="/register"
                onClick={handleLinkClick}
                className="w-full text-center mt-2 text-sm text-zinc-900 hover:text-amber-700 flex items-center justify-center gap-1"
              >
                <UserPlus size={16} /> Crear una cuenta
              </Link>
            </div>
...
const LinkItem = ({ to, onClick, Icon, children }) => (
  <li className="text-sm group">
    <Link
      to={to}
      onClick={onClick}
      className="text-stone-700 hover:text-amber-700 transition-colors flex items-center gap-3 py-1 font-medium hover:bg-amber-50 rounded-md px-2 -mx-2"
    >
      <Icon
        size={18}
        className="text-stone-500 group-hover:text-amber-700 transition-colors"
      />
      {children}
    </Link>
  </li>
